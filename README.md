...
```
    ...
    ## Called from: mgpEstimate(exprData = mgp_LesnickParkinsonsExp, genes = mouseMarkerGenes$Midbrain, 
    ##     geneColName = "Gene.Symbol", outlierSampleRemove = F, geneTransform = function(x) {
    ##         homologene::mouse2human(x)$humanGene
    ##     }, groups = mgp_LesnickParkinsonsMeta$disease, seekConsensus = FALSE, 
    ##     removeMinority = TRUE)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#228: if (!is.null(geneTransform)) {
    ##     transformedGenes = genes %>% lapply(geneTransform)
    ##     dataGenes = exprData[[geneColName]]
    ##     if (sum(unlist(transformedGenes) %in% dataGenes) < (sum(unlist(genes) %in% 
    ##         dataGenes))) {
    ...
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#229: transformedGenes = genes %>% lapply(geneTransform)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#230: dataGenes = exprData[[geneColName]]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#232: if (sum(unlist(transformedGenes) %in% dataGenes) < (sum(unlist(genes) %in% 
    ##     dataGenes))) {
    ##     warning("geneTransform function reduces the number of matches between marker gene list and the expression data. Default method is to transform mouse genes to human genes. If this is not what you want please set geneTransform to NULL. Please read to documentary to make sure you are doing it right.")
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#235: genes = transformedGenes
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#239: if (is.null(groups)) {
    ##     assertthat::assert_that(seekConsensus == FALSE & is.null(groups))
    ##     list[, exp] = ogbox::sepExpr(exprData)
    ##     groups = rep(1, ncol(exp))
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#244: if (!is.null(indivGenePlot[1])) {
    ##     if (length(indivGenePlot) == 1) {
    ##         indivGenePlot = paste0(indivGenePlot, "/", names(genes), 
    ##             "indivExp.png")
    ##     }
    ##     toCreate = unique(dirname(indivGenePlot))
    ##     sapply(toCreate, dir.create, showWarnings = F, recursive = T)
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#251: if (!is.null(tableOut[1])) {
    ...
    ##     sapply(toCreate, dir.create, showWarnings = F, recursive = T)
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#259: if (typeof(genes) != "list") {
    ##     genes = list(genes)
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#263: if (seekConsensus) {
    ##     groupRotations = groupRotations(exprData, genes, geneColName, 
    ##         groups, outDir = NULL, geneTransform = NULL)
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#269: estimateOut = vector(mode = "list", length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#270: groupsOut = vector(mode = "list", length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#271: rotations = vector(mode = "list", length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#273: trimmedPCAs = vector(mode = "list", length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#274: fullPCAs = vector(mode = "list", length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#275: removedMarkerRatios = vector(length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#277: meanUsedMarkerExpression = vector(mode = "list", length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#278: usedMarkerExpression = vector(mode = "list", length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#279: simpleScaledEstimation = vector(mode = "list", length = length(genes))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#282: for (i in 1:length(genes)) {
    ...
    ##             genes[[i]] = rownames(groupRotations[[i]][apply(groupRotations[[i]], 
    ##                 1, function(x) {
    ##                   all(x > 0)
    ##                 }), ])
    ##         }
    ...
    ##         indivGenes = indivGenes %>% dplyr::mutate(group = groups[match(GSM, 
    ##             colnames(tempExp))]) %>% dplyr::mutate(gene = factor(gene, 
    ##             levels = genes[[i]]) %>% droplevels)
    ##         switch(plotType[1], groupBased = {
    ##         }, cummulative = {
    ##             indivGenes$group = ""
    ##         })
    ##         p = ggplot2::ggplot(indivGenes, ggplot2::aes(y = expression, 
    ##             x = group)) + ggplot2::facet_wrap("gene") + ggplot2::geom_boxplot(fill = "lightblue") + 
    ##             ggplot2::geom_point() + ggplot2::theme(axis.text.x = ggplot2::element_text(size = 20, 
    ##             angle = 90), axis.title.y = ggplot2::element_text(vjust = 0.5, 
    ##             size = 20), axis.title.x = ggplot2::element_text(vjust = 0.5, 
    ##             size = 0), title = ggplot2::element_text(vjust = 0.5, 
    ##             size = 20)) + ggplot2::scale_x_discrete(name = "") + 
    ##             ggplot2::scale_y_discrete(name = "log2 Expression")
    ##         (p)
    ##         ggplot2::ggsave(filename = indivGenePlot[i], width = 8, 
    ##             height = 8)
    ##     }
    ##     pca = stats::prcomp(t(relevantExpr), scale = TRUE)
    ##     trimmedPCA = pca
    ##     pca$rotation = pca$rotation * ((sum(pca$rotation[, PC]) < 
    ##         0) * (-2) + 1)
    ##     if (removeMinority) {
    ##         while (any(pca$rotation[, PC] < 0)) {
    ##             relevantExpr = relevantExpr[pca$rotation[, PC] > 
    ##                 0, ]
    ##             pca = stats::prcomp(t(relevantExpr), scale = T)
    ##             trimmedPCA = pca
    ##             pca$rotation = pca$rotation * ((sum(pca$rotation[, 
    ##                 PC]) < 0) * (-2) + 1)
    ##         }
    ##     }
    ...
    ##         writeLines(paste0("# Variation explained: ", paste(summary(pca)$importance[2, 
    ##             ], collapse = " ")), fileConn)
    ##         close(fileConn)
    ##         utils::write.table(pca$rotation[, PC, drop = F], file = tableOut[i], 
    ##             quote = F, row.names = T, col.names = F, sep = "\t", 
    ##             append = T)
    ##     }
    ##     pca$x = t(as.matrix(t(scale(t(relevantExpr))))) %*% as.matrix(pca$rotation)
    ##     simpleScaleWeights = ogbox::scale01(pca$rotation[, PC])/(sum(ogbox::scale01(pca$rotation[, 
    ##         PC])))
    ##     simpleScale = apply((relevantExpr * simpleScaleWeights), 
    ##         2, sum)
    ##     simpleScaledEstimation[[i]] = simpleScale
    ##     markerGeneExpression = relevantExpr
    ##     usedMarkerExpression[[i]] = relevantExpr
    ##     meanUsedMarkerExpression[[i]] = relevantExpr %>% apply(2, 
    ##         mean)
    ##     groupsOut[[i]] = groups
    ...
    ##         pca$x = pca$x[!rownames(pca$x) %in% names(box$out), , 
    ##             drop = F]
    ##     }
    ##     estimateOut[[i]] = (pca$x[, PC])
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#284: relevantData = exprData[exprData[, geneColName] %in% genes[[i]], 
    ##     ]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#285: rownames(relevantData) = relevantData[, geneColName]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#286: list[, relevantExpr] = ogbox::sepExpr(relevantData)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#287: if (nrow(relevantData) == 0) {
    ##     estimateOut[[i]] = NA
    ##     groupsOut[[i]] = NA
    ##     next
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#293: pca = stats::prcomp(t(relevantExpr), scale = TRUE)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#294: originalPCA = pca
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#295: fullPCAs[[i]] = originalPCA
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#299: if (seekConsensus) {
    ##     if (!is.na(groupRotations[i])) {
    ##         genes[[i]] = rownames(groupRotations[[i]][apply(groupRotations[[i]], 
    ##             1, function(x) {
    ##                 all(x > 0)
    ...
    ##     }
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#305: relevantData = exprData[exprData[, geneColName] %in% genes[[i]], 
    ##     ]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#307: if (nrow(relevantData) == 0) {
    ##     estimateOut[[i]] = NA
    ##     groupsOut[[i]] = NA
    ##     next
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#313: rownames(relevantData) = relevantData[, geneColName]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#314: list[, relevantExpr] = ogbox::sepExpr(relevantData)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#317: if (!is.null(indivGenePlot[1])) {
    ##     tempExp = relevantExpr
    ##     tempExp$gene = rownames(tempExp)
    ##     indivGenes = reshape2::melt(tempExp)
    ##     names(indivGenes) = c("gene", "GSM", "expression")
    ...
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#350: pca = stats::prcomp(t(relevantExpr), scale = TRUE)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#351: trimmedPCA = pca
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#352: pca$rotation = pca$rotation * ((sum(pca$rotation[, PC]) < 0) * 
    ##     (-2) + 1)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#355: if (removeMinority) {
    ##     while (any(pca$rotation[, PC] < 0)) {
    ...
    ##     }
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#356: while (any(pca$rotation[, PC] < 0)) {
    ##     relevantExpr = relevantExpr[pca$rotation[, PC] > 0, ]
    ##     pca = stats::prcomp(t(relevantExpr), scale = T)
    ##     trimmedPCA = pca
    ##     pca$rotation = pca$rotation * ((sum(pca$rotation[, PC]) < 
    ##         0) * (-2) + 1)
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#357: relevantExpr = relevantExpr[pca$rotation[, PC] > 0, ]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#358: pca = stats::prcomp(t(relevantExpr), scale = T)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#359: trimmedPCA = pca
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#360: pca$rotation = pca$rotation * ((sum(pca$rotation[, PC]) < 0) * 
    ##     (-2) + 1)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#356: (while) any(pca$rotation[, PC] < 0)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#357: relevantExpr = relevantExpr[pca$rotation[, PC] > 0, ]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#358: pca = stats::prcomp(t(relevantExpr), scale = T)
    ...
    ##     (-2) + 1)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#356: (while) any(pca$rotation[, PC] < 0)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#364: trimmedPCAs[[i]] = trimmedPCA
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#366: ngenes = trimmedPCA$rotation %>% nrow
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#367: ngenesBefore = originalPCA$rotation %>% nrow
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#369: removedMarkerRatios[[i]] = (ngenesBefore - ngenes)/ngenesBefore
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#372: rotations[[i]] = pca$rotation
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#375: if (!is.null(tableOut[1])) {
    ##     file.create(tableOut[i])
    ##     fileConn = file(tableOut[i])
    ##     writeLines(paste0("# Variation explained: ", paste(summary(pca)$importance[2, 
    ##         ], collapse = " ")), fileConn)
    ...
    ##         append = T)
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#391: pca$x = t(as.matrix(t(scale(t(relevantExpr))))) %*% as.matrix(pca$rotation)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#394: simpleScaleWeights = ogbox::scale01(pca$rotation[, PC])/(sum(ogbox::scale01(pca$rotation[, 
    ##     PC])))
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#395: simpleScale = apply((relevantExpr * simpleScaleWeights), 2, sum)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#396: simpleScaledEstimation[[i]] = simpleScale
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#398: markerGeneExpression = relevantExpr
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#401: usedMarkerExpression[[i]] = relevantExpr
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#402: meanUsedMarkerExpression[[i]] = relevantExpr %>% apply(2, mean)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#404: groupsOut[[i]] = groups
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#406: if (outlierSampleRemove) {
    ##     warning("Outlier sample removal is deprecated and will be removed. If you were using it and this caused problems for you this please mail ogan.mancarcii@gmail.com")
    ##     groupData = sapply(unique(groups), function(x) {
    ##         pca$x[groups %in% x, PC]
    ##     }, simplify = F)
    ...
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#418: estimateOut[[i]] = (pca$x[, PC])
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#284: relevantData = exprData[exprData[, geneColName] %in% genes[[i]], 
    ##     ]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#285: rownames(relevantData) = relevantData[, geneColName]
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#286: list[, relevantExpr] = ogbox::sepExpr(relevantData)
    ...
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#418: estimateOut[[i]] = (pca$x[, PC])
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#420: names(usedMarkerExpression) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#421: names(meanUsedMarkerExpression) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#423: names(estimateOut) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#424: names(groupsOut) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#425: names(rotations) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#427: names(trimmedPCAs) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#428: names(fullPCAs) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#429: names(removedMarkerRatios) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#430: names(simpleScaledEstimation) = names(genes)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#432: problematic = which(removedMarkerRatios > 0.4) %>% names
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#433: if (length(problematic) > 0) {
    ##     warning(paste0("Several cell types have a high proportion (>0.4) of their genes filtered out. Excercise caution.\nProblematic cell types are: ", 
    ##         paste(problematic, collapse = ", ")))
    ## }
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#439: output = list(estimates = estimateOut, groups = groupsOut, rotations = rotations, 
    ##     trimmedPCAs = trimmedPCAs, fullPCAs = fullPCAs, removedMarkerRatios = removedMarkerRatios, 
    ##     usedMarkerExpression = usedMarkerExpression, meanUsedMarkerExpression = meanUsedMarkerExpression, 
    ##     simpleScaledEstimation = simpleScaledEstimation)
    ## debug at /home/omancarci/git repos/markerGeneProfile/R/mgpEstimation.R#449: return(output)
